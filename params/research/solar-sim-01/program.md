...
# Research job: solar-sim-01

## Goal
Lower `mean(|timing_offset_min|)` on the solar_eclipse subset by editing
`params/research/solar-sim-01/current.json`. Lower is better.

## The loop
1. Form a hypothesis about which allowlisted parameter(s) to change and why.
2. Edit `current.json` (only keys in the allowlist).
3. Run `./research iterate solar-sim-01` and read `objective: <n>`.
4. If lower than current best: run `./research validate solar-sim-01`
